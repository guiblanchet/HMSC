...
##########################################################################################
	...
	for(i in 1:niter){
		if(!is.null(data$Random)){
			...
			for(j in 1:nRandom){
				...
				if(nRandomLev[j]==nsite){
					...
					RprecLatent <- qr.R(qr(cholprecLatent)) ### Cette ligne peut-être problématique car les signes ne sont pas toujours bon... mais bon... à voir si ça cause un problème plus loin
		...
		#___________________
		if(!is.logical(outlierSp)){
			shapeOutlierSp<-(priors$outlierSp+nparamX)/2 ### Peut être déplacer à l'extérieur de la boucle for()
			for(j in 1:nsp){
				paramXct<-param$paramX[j,]-param$meansparamX[j,]
				rateOutlierSp<-0.5*(priors$outlierSp+paramXct%*%param$precX%*%paramXct)
				outlierSp[j]<-rgamma(1,shape=shapeOutlierSp,rate=rateOutlierSp) ### Peut-être que "rate" devrait être "shape"... Mais je crois que c'est bon !
			}
		}
		...
		if(!is.null(data$Random)){
			for(j in 1:nRandom){
				...
				shrinkLocal[[j]]<-matrix(rgamma(nsp*nLatent[j],shape=priors$shrinkLocal/2+0.5,rate=rateShrinkLocal),nrow=nsp,ncol=nLatent[j]) ### Je suis pas mal certain que c'est bon mais à reviser juste au cas où il y aurait un problème (pas une priorité !)
				...
				paramShrinkGlobal[[j]][1]<-rgamma(1,shape=shapeParamShrinkGlobal,rate=rateParamShrinkGlobal) ### ** À réviser si quelques chose cloches (mais je crois que c'est bon)
				...
				for(h in 2:nLatent[j]){
					...
					paramShrinkGlobal[[j]][h]<-rgamma(1,shape=shapeParamShrinkGlobal,rate=rateParamShrinkGlobal) ### ** À réviser si quelques chose cloches (mais je crois que c'est bon)
					shrinkGlobal[[j]]<-cumprod(paramShrinkGlobal[[j]])
				}
