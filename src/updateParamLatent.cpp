...
arma::field<arma::mat> updateParamLatent(arma::mat& Ylatent,
	...
	for (int j = 0; j < nRandom; j++) {
		...
		mat latentCross = trans(latent(j,0).rows(Random.col(j)))*latent(j,0).rows(Random.col(j));
		for (int k = 0; k < nsp; k++) {
			shrinkLatent = diagmat(shrink(j,0).row(k))+residVar(k)*latentCross;
			mat latentSp = residVar(k)*trans(latent(j,0).rows(Random.col(j)))*Yresid.col(k);
